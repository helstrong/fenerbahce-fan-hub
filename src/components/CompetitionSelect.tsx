...
export default function CompetitionSelect({
  ...
  return (
    <select
      ...
      className={`rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-fener-navy shadow-sm transition focus:outline-none focus:ring-2 focus:ring-fener-yellow ${className}`}
    >
      {competitions.map((c) => (
